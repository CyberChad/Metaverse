...
# TODO eventually class ActrAgent(CommonAgent):

"""
cmuactr() maintains dispatch command registration
"""
...
class cmuactr():
    ...
    def add_command(self, name, function, documentation="No documentation provided.", single=True, cmuactr_name=None):
        ...
        if existing[0] == True:
            if existing[1] == None:
                result = self.interface.send("add", name, name, documentation, single, cmuactr_name)
                if result[0]:
                    self.interface.add_command(name, function)
                    return result[1]
                else:
...
"""
start(), connection(), and stop() the connection() using interface()
"""
        ...
        if port == None and os.path.isfile(portfile):
            with open(portfile, 'r') as f:
                try:
                    ...
                    print("Problem reading ACT-R_CMU port number from", portfile, ". Using default or 2650.")
                    port = 2650
        elif port == None:
            print("ACT-R_CMU port number file", portfile, "not found. Using default or 2650.")
            port = 2650
        ...
        if host == None and os.path.isfile(hostfile):
            with open(hostfile, 'r') as f:
                try:
                    ...
                    print("Problem reading ACT-R_CMU host from", hostfile, ". Using default of 127.0.0.1.")
                    host = "127.0.0.1"
        elif host == None:
            print("ACT-R_CMU host file", hostfile, "not found. Using default of 127.0.0.1.")
            host = "127.0.0.1"

        try:
            a = cmuactr(host=host, port=port)
        except:
            print("Failed to connect to ACT-R_CMU with exception", sys.exc_info())

        else:
            if a.interface.connected:
                a.interface.send("set-name", "ACT-R_CMU Tutorial Python interface")
                current_connection = a
                return current_connection
            else:
                print("ACT-R_CMU connection NOT established, but no exception detected or already handled.")
    else:
        print("ACT-R_CMU is already connected.")
        return current_connection
...
def connection():
    if current_connection == None:
        s = start()
        if s:
            print("ACT-R_CMU connection has been started.")
            return s
        else:
            print("Could not start ACT-R_CMU connection.")
    else:
        return current_connection
...
def stop():
    ...
    if current_connection == None:
        print("No current ACT-R_CMU connection to stop.")
    else:
        print("Closing down ACT-R_CMU connection.")
...
class interface():
    def __init__(self, host, port):
        ...
        try:
            ...
            print("Error trying to connect to ACT-R_CMU at", host, ":", port, "with exception", sys.exc_info())
        else:
            self.connected = True
            self.cmd_id = 1
            self.actions = {}
    ...
    def collect_data(self):
        ...
        while c:
            try:
                ...
                if self.connected:
                    print("ACT-R_CMU connection error connection no longer available.")
                c = False
...
"""
The following sections are organized by functional group and aligned 
as much as possible with the ACT-R Reference Manual (v7.13)
"""

# ***********************************
#       Loading and Running System Commands
# **********************************
